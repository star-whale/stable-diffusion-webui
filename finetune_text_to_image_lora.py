...
def fine_tune(context: Context,
    ...
    def save_lora(origin_file: os.PathLike, target_file: str):
        checkpoint = torch.load(origin_file, map_location=device)
        ...
        torch_to_st.save_file(new_dict, target_file)

    # convert the model to Safetensors format and save it to a file
    unet.save_attn_procs(save_directory=model_dir)
    # save safetensors to lora dir TODO use root or other dir?
    save_lora(model_dir / LORA_WEIGHT_NAME, str(model_dir / "Lora" / LORA_WEIGHT_NAME_SAFE))

    from swmp import ft
    model.build(
        workdir=ROOT_DIR,
        name="SD-finetune",
        modules=[ft],  # TODO use eval handler
    )
